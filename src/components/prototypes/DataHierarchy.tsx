  ...
  const loadData = async () => {
    try {
      ...
      const tableQueries = [
        ...
        { name: 'children', query: supabase.from('children').select('*').limit(10) },
        { name: 'wellbeing_sessions', query: supabase.from('wellbeing_sessions').select('*').limit(10) },
        { name: 'wellbeing_responses', query: supabase.from('wellbeing_responses').select('*').limit(10) },
        { name: 'checkin_sessions', query: supabase.from('checkin_sessions').select('*').limit(10) },
        { name: 'breathing_tool_data', query: supabase.from('breathing_tool_data').select('*').limit(10) },
        { name: 'mood_meter_data', query: supabase.from('mood_meter_data').select('*').limit(10) },
        { name: 'emotion_grid_data', query: supabase.from('emotion_grid_data').select('*').limit(10) },
        { name: 'wellbeing_wheel_data', query: supabase.from('wellbeing_wheel_data').select('*').limit(10) },
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mt-8 bg-blue-900/20 border border-blue-500 rounded-lg p-4">
          ...
          <div className="text-sm text-gray-300 space-y-2">
            <p>If tables are missing, you may need to run the database migrations:</p>
            <code className="block bg-gray-800 p-2 rounded mt-2">
              npx supabase db push
            </code>
            <p className="mt-3">Or manually create tables in your Supabase dashboard.</p>
          </div>
        </div>
      </div>
    </div>
